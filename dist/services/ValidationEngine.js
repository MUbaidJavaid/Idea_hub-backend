...
function expCurve(raw, scale) {
    return Math.round(100 * (1 - Math.exp(-raw / scale)));
}
function scoreCommunity(likeCount, commentCount, pollVoteCount) {
    const raw = likeCount * 2.5 + commentCount * 3 + pollVoteCount * 4;
    return clamp(expCurve(raw, 35), 0, 100);
}
function scoreCollaboratorWant(collaboratorCount, pendingCollabRequests) {
...
function scoreCompleteness(idea) {
...
async function heuristicAiAnalysis(title, description) {
    const text = `${title}\n${description}`.toLowerCase();
    const len = text.length;
    const marketScore = clamp(35 + Math.round(Math.min(40, len / 80)) + (text.includes('market') ? 8 : 0), 0, 100);
    const marketSize = len > 1200 ? 'large' : len > 500 ? 'medium' : 'small';
    const competition = /\b(many|crowded|competitors|saturated)\b/.test(text)
        ? 'high'
        : /\b(niche|unique|first)\b/.test(text)
            ? 'low'
            : 'medium';
    const feasibility = /\b(complex|years|research-heavy)\b/.test(text)
        ? 'hard'
        : /\b(mvp|simple|weekend)\b/.test(text)
            ? 'easy'
            : 'medium';
    const timing = /\b(future|2030|not ready)\b/.test(text)
        ? 'too_early'
...
async function checkUniqueness(ideaId, title, tags) {
...
export function scheduleValidationRecalculate(ideaId, options) {
    if (!isEngineEnabled())
        return;
    ...
    if (prev)
        clearTimeout(prev);
    debounceTimers.set(ideaId, setTimeout(() => {
...
export async function calculateScore(ideaId, options) {
    if (!isEngineEnabled())
        return null;
    if (!mongoose.Types.ObjectId.isValid(ideaId))
        return null;
    const oid = new mongoose.Types.ObjectId(ideaId);
    const idea = await Idea.findById(oid).lean();
    if (!idea)
        return null;
    ...
    const communityVotes = scoreCommunity(idea.likeCount ?? 0, idea.commentCount ?? 0, pollVoteCount);
    const collaboratorWant = scoreCollaboratorWant(idea.collaborators?.length ?? 0, pendingCollabs);
    const uniquenessScore = await checkUniqueness(oid, idea.title, idea.tags ?? []);
    const completenessScore = scoreCompleteness(idea);
    const forceAi = Boolean(options?.forceAi);
    const dayMs = 24 * 60 * 60 * 1000;
    const needsFreshAi = forceAi ||
        !lastAi ||
    ...
    const total = clamp(Math.round(communityVotes * 0.25 +
        collaboratorWant * 0.2 +
        aiMarketScore * 0.25 +
        uniquenessScore * 0.15 +
        completenessScore * 0.15), 0, 100);
    let trend = 'stable';
