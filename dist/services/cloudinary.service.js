...
export class CloudinaryConfigError extends Error {
    constructor() {
        super('Cloudinary is not configured. Set CLOUDINARY_CLOUD_NAME, CLOUDINARY_API_KEY, CLOUDINARY_API_SECRET.');
 ...
 */
function cloudinaryCallbackErrorMessage(err) {
...
export async function uploadToCloudinary(input) {
    ...
    if (!effectiveMime || !ALLOWED_MIME.has(effectiveMime)) {
        throw new Error(`Unsupported or unverified file type${detected?.mime ? `: ${detected.mime}` : ''}`);
    }
    ...
    const result = await new Promise((resolve, reject) => {
        ...
        const stream = upload(uploadOptions, (err, res) => {
            if (err || !res) {
                if (err) {
