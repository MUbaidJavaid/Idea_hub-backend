...
function isDocumentMediaItem(item) {
    if (['pdf', 'doc', 'spreadsheet'].includes(item.mediaType))
        return true;
...
export async function processScanJob(job) {
    ...
    for (let i = 0; i < mediaItems.length; i += 1) {
        ...
        await Idea.updateOne({ _id: ideaId, 'media._id': mediaObjectId }, {
            $set: {
    ...
    if (isSevereViolation) {
        await Idea.findByIdAndUpdate(ideaId, {
            $set: {
                ...
                rejectionReason: `Content policy: ${aggregate.allViolations.slice(0, 3).join(', ')}`.slice(0, 2000),
            },
        });
        await db.ref(`scan_updates/${ideaId}`).set({
            status: 'flagged',
        ...
        await Notification.create({
            ...
            body: `Our automated review flagged this content (${preview}). You can appeal from your drafts or contact support.`.slice(0, 500),
            isRead: false,
            isPushSent: false,
            metadata: {
                violations: aggregate.allViolations,
...
export function createScannerWorker() {
    ...
    worker.on('failed', (job, err) => {
        console.error(`[scanner] job ${job?.id ?? 'unknown'} failed`, err?.message ?? err);
    });
