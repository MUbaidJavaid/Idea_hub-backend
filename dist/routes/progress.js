...
import { currentWeekBucket, ensureUserProgress, getLeaderboard, getUserRank, isGamificationEnabled, onValidationVote, progressToApi, recordDailyActivity, } from '../services/gamification.service.js';
...
function gamificationOr404(res) {
...
progressRouter.get('/leaderboard', requireDb, optionalAuth, async (req, res) => {
    if (!gamificationOr404(res))
        return;
    const scopeRaw = typeof req.query.scope === 'string' ? req.query.scope.trim() : 'global';
    ...
    const meId = res.locals.authUserId;
    let myRank = null;
    let myRow = null;
...
progressRouter.post('/validation-vote', requireDb, requireAuth, async (req, res) => {
    if (!gamificationOr404(res))
        return;
    const userId = res.locals.authUserId;
    if (!userId || !mongoose.Types.ObjectId.isValid(userId)) {
