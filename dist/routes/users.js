...
const FOLLOW_POPULATE_SELECT = 'username email fullName bio avatarUrl role status isEmailVerified skills followerCount followingCount totalIdeasPosted notificationPreferences createdAt verifiedInnovator';
async function listFollowersPage(targetUserId, cursor) {
    const filter = { followingId: targetUserId };
    ...
    const users = page
        .map((d) => d.followerId)
        .filter((u) => Boolean(u && typeof u === 'object' && '_id' in u));
    return { users, nextCursor };
}
async function listFollowingPage(targetUserId, cursor) {
    const filter = { followerId: targetUserId };
    if (cursor && mongoose.Types.ObjectId.isValid(cursor)) {
        filter._id = { $lt: new mongoose.Types.ObjectId(cursor) };
    }
    const docs = await Follow.find(filter)
...
usersRouter.patch('/me', requireDb, requireAuth, async (req, res) => {
    const userId = res.locals.authUserId;
    if (!userId || !mongoose.Types.ObjectId.isValid(userId)) {
        res.status(401).json({
            success: false,
    ...
    const usernameIn = typeof body.username === 'string'
        ? body.username.trim().toLowerCase()
        : undefined;
    const avatarUrlIn = typeof body.avatarUrl === 'string' ? body.avatarUrl.trim().slice(0, 2048) : undefined;
...
usersRouter.post('/me/verification-request', requireDb, requireAuth, async (req, res) => {
    const userId = res.locals.authUserId;
    if (!userId || !mongoose.Types.ObjectId.isValid(userId)) {
        res.status(401).json({
            success: false,
...
usersRouter.get('/me/notifications', requireDb, requireAuth, async (req, res) => {
    ...
    const docs = await Notification.find(filter)
        ...
        .populate({
        path: 'senderId',
        select: 'username email fullName bio avatarUrl role status isEmailVerified skills followerCount followingCount totalIdeasPosted notificationPreferences createdAt',
    });
    const hasMore = docs.length > LIST_PAGE;
    const page = hasMore ? docs.slice(0, LIST_PAGE) : docs;
    const nextCursor = hasMore && page.length > 0
    ...
    const data = page.map((doc) => {
        ...
        const sender = s && typeof s === 'object' && '_id' in s
            ? s
            : null;
...
usersRouter.patch('/me/notifications/read-all', requireDb, requireAuth, async (req, res) => {
    const userId = res.locals.authUserId;
    if (!userId || !mongoose.Types.ObjectId.isValid(userId)) {
        res.status(401).json({
            success: false,
...
usersRouter.patch('/me/notifications/:id/read', requireDb, requireAuth, async (req, res) => {
    const userId = res.locals.authUserId;
    const { id } = req.params;
    if (!userId || !mongoose.Types.ObjectId.isValid(userId)) {
        res.status(401).json({
...
usersRouter.post('/me/saved-ideas/:ideaId', requireDb, requireAuth, async (req, res) => {
    const userId = res.locals.authUserId;
...
usersRouter.delete('/me/saved-ideas/:ideaId', requireDb, requireAuth, async (req, res) => {
    const userId = res.locals.authUserId;
    const { ideaId } = req.params;
    if (!userId || !mongoose.Types.ObjectId.isValid(userId)) {
        res.status(401).json({
...
usersRouter.get('/me/followers', requireDb, requireAuth, async (req, res) => {
    ...
    const cursor = typeof req.query.cursor === 'string' ? req.query.cursor.trim() : '';
    const { users, nextCursor } = await listFollowersPage(new mongoose.Types.ObjectId(userId), cursor);
    res.json({
        success: true,
        message: 'OK',
...
usersRouter.get('/me/following', requireDb, requireAuth, async (req, res) => {
    ...
    const cursor = typeof req.query.cursor === 'string' ? req.query.cursor.trim() : '';
    const { users, nextCursor } = await listFollowingPage(new mongoose.Types.ObjectId(userId), cursor);
    res.json({
        success: true,
        message: 'OK',
        data: users.map((u) => userToApiPublic(u)),
...
usersRouter.post('/:userId/follow', requireDb, requireAuth, async (req, res) => {
...
usersRouter.delete('/:userId/follow', requireDb, requireAuth, async (req, res) => {
    const followerId = res.locals.authUserId;
    const { userId: targetId } = req.params;
    if (!followerId ||
        !mongoose.Types.ObjectId.isValid(followerId) ||
...
usersRouter.get('/:userId/ideas', requireDb, optionalAuth, async (req, res) => {
    ...
    const cursor = typeof req.query.cursor === 'string' ? req.query.cursor.trim() : '';
    const viewerId = res.locals.authUserId;
...
usersRouter.get('/:username/followers', requireDb, optionalAuth, async (req, res) => {
    ...
    const cursor = typeof req.query.cursor === 'string' ? req.query.cursor.trim() : '';
    const { users, nextCursor } = await listFollowersPage(user._id, cursor);
    res.json({
        success: true,
        message: 'OK',
        data: users.map((u) => userToApiPublic(u)),
...
usersRouter.get('/:username/following', requireDb, optionalAuth, async (req, res) => {
    ...
    const cursor = typeof req.query.cursor === 'string' ? req.query.cursor.trim() : '';
    const { users, nextCursor } = await listFollowingPage(user._id, cursor);
    res.json({
        success: true,
        message: 'OK',
        data: users.map((u) => userToApiPublic(u)),
