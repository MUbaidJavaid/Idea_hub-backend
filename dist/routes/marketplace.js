...
import { netToSellerUsd, platformFeeFromSaleUsd, SUBSCRIPTION_PRICES_USD, } from '../config/monetization.config.js';
import { getEffectivePlan } from '../lib/subscription.js';
import { ideaPreviewForListing, listingToApi, } from '../lib/serialize-marketplace.js';
import { userToApiPublic } from '../lib/serialize-user.js';
import { optionalAuth } from '../middleware/optional-auth.js';
import { requireAuth } from '../middleware/require-auth.js';
import { Idea, MarketplaceInterest, MarketplaceListing, User, } from '../models/index.js';
import { assertNoConflictingActiveListing, defaultListingExpiresAt, loadListingForSeller, notifyBidderOutcome, notifyMarketplaceBid, notifyMarketplaceInterest, validStatusTransition, } from '../services/marketplace.service.js';
...
async function ideaIdsForBrowseFilters(params) {
    if (!params.category && params.validationMin == null)
        return null;
    const q = {
        status: 'published',
        visibility: 'public',
...
async function attachListingPayload(doc, viewerUserId) {
...
listingsRouter.get('/featured', requireDb, async (_req, res) => {
    ...
    const docs = await MarketplaceListing.find({
        ...
        .limit(12)
        .exec();
    const out = await Promise.all(docs.map((d) => attachListingPayload(d, null)));
...
listingsRouter.get('/my', requireDb, requireAuth, async (req, res) => {
    ...
    const docs = await MarketplaceListing.find({
        ...
        .limit(100)
        .exec();
    const out = await Promise.all(docs.map((d) => attachListingPayload(d, userId)));
    res.json({ success: true, message: 'OK', data: out });
});
listingsRouter.post('/', requireDb, requireAuth, async (req, res) => {
    ...
    const targetBuyer = typeof body.targetBuyer === 'string' ? body.targetBuyer.trim() : '';
    const proofPoints = Array.isArray(body.proofPoints)
        ? body.proofPoints.map((x) => String(x).trim()).filter(Boolean).slice(0, 40)
        : [];
    const askingPrice = typeof body.askingPrice === 'number' && body.askingPrice >= 0
        ? body.askingPrice
        : 0;
    const equity = typeof body.equity === 'number' && body.equity >= 0 && body.equity <= 100
        ? body.equity
        : 0;
    ...
    if (!listingType || !allowedTypes.includes(listingType)) {
        res.status(400).json({
            success: false,
    ...
    if (effPlan === 'free') {
        res.status(403).json({
            success: false,
            message: 'Marketplace listings require a Pro or Investor subscription. See Pricing to upgrade.',
            data: null,
        });
        return;
    }
    if (effPlan === 'pro') {
        ...
        if (listingCount >= 1) {
            res.status(403).json({
                success: false,
                message: 'Pro includes 1 marketplace listing. Upgrade to Investor for unlimited listings.',
                data: null,
            });
            return;
        }
...
listingsRouter.get('/', requireDb, optionalAuth, async (req, res) => {
    const viewerId = res.locals.authUserId ?? null;
    const cursor = typeof req.query.cursor === 'string' ? req.query.cursor.trim() : '';
    const listingType = typeof req.query.listingType === 'string'
    ...
    const out = await Promise.all(page.map((d) => attachListingPayload(d, viewerId)));
    res.json({
        success: true,
        message: 'OK',
        data: out,
...
listingsRouter.patch('/:id', requireDb, requireAuth, async (req, res) => {
    ...
    if (nextStatus === 'active') {
        const ok = await assertNoConflictingActiveListing(doc.ideaId, doc._id);
        if (!ok) {
            res.status(409).json({
                success: false,
...
listingsRouter.get('/:id', requireDb, optionalAuth, async (req, res) => {
    ...
    const isPublic = doc.status === 'active' &&
...
listingsRouter.post('/:id/bid', requireDb, requireAuth, async (req, res) => {
    ...
    const bidderName = bidder?.fullName || bidder?.username || 'A buyer';
...
listingsRouter.patch('/:id/bids/:bidId', requireDb, requireAuth, async (req, res) => {
    const userId = res.locals.authUserId;
    ...
    const bid = doc.bids.find((b) => String(b._id) === bidId);
...
listingsRouter.post('/:id/interest', requireDb, requireAuth, async (req, res) => {
    ...
    res.status(201).json({
        success: true,
        message: 'OK',
        data: await attachListingPayload((await MarketplaceListing.findById(id)), userId),
    });
});
    ...
    res.json({
        ...
        data: {
            sales: sales.map((s) => ({
                ...
                soldAt: s.soldAt instanceof Date ? s.soldAt.toISOString() : s.soldAt ?? null,
            })),
            totals: {
...
marketplaceRouter.post('/seller/payout/connect', requireDb, requireAuth, async (_req, res) => {
    res.json({
        success: true,
