...
import { CollectionFollow, Idea, IdeaCollection, IdeaCollectionItem, User, } from '../models/index.js';
...
async function mapCollectionIdeas(docs, viewerUserId) {
    if (docs.length === 0)
        return [];
...
collectionsRouter.get('/by-user/:username', requireDb, optionalAuth, async (req, res) => {
...
function collectionToApi(c, following) {
    return {
        _id: String(c._id),
...
collectionsRouter.get('/:id', requireDb, optionalAuth, async (req, res) => {
    ...
    if (viewer && mongoose.Types.ObjectId.isValid(viewer)) {
        following = Boolean(await CollectionFollow.exists({
            followerId: viewer,
    ...
    const byId = new Map(ideas.map((idea) => [String(idea._id), idea]));
...
collectionsRouter.delete('/:id/items/:ideaId', requireDb, requireAuth, async (req, res) => {
...
collectionsRouter.post('/:id/follow', requireDb, requireAuth, async (req, res) => {
    ...
    try {
        ...
        await IdeaCollection.updateOne({ _id: col._id }, { $inc: { followerCount: 1 } });
    }
    catch (e) {
        const dup = typeof e === 'object' &&
            e !== null &&
...
collectionsRouter.delete('/:id/follow', requireDb, requireAuth, async (req, res) => {
    ...
    if (del.deletedCount > 0) {
        await IdeaCollection.updateOne({ _id: col._id, followerCount: { $gt: 0 } }, { $inc: { followerCount: -1 } });
    }
    const fresh = await IdeaCollection.findById(col._id);
    res.json({
        success: true,
