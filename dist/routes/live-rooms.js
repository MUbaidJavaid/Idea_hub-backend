...
import { LIVE_FREE_MAX_PARTICIPANTS, liveRoomProvider, liveRoomsEnabled, } from '../config/live.config.js';
...
import { Idea, LiveRoom, LiveRoomMessage, LiveRoomQuestion, LiveRoomRsvp, User, } from '../models/index.js';
import { dailyCreateRoom, dailyMeetingToken, dailyRoomJoinUrl } from '../services/daily-live.service.js';
import { attachLiveRecordingToIdea, notifyLiveRoomStarted, } from '../services/live-rooms.service.js';
...
function requireLive(_req, res, next) {
    if (!liveRoomsEnabled()) {
        res.status(404).json({
            success: false,
...
function listenerCount(room) {
    return (room.participants ?? []).filter((p) => !p.leftAt && p.role === 'listener').length;
}
function findOpenParticipant(room, userId) {
    return (room.participants ?? []).find((p) => String(p.userId) === String(userId) && !p.leftAt);
}
...
liveRoomsRouter.get('/', optionalAuth, async (req, res) => {
    ...
    const rooms = await LiveRoom.find(filter)
        ...
        .limit(24);
    const data = await mapLiveRoomsToApi(rooms);
    const nextCursor = rooms.length > 0 ? String(rooms[rooms.length - 1]._id) : undefined;
    res.json({
        success: true,
        message: 'OK',
        data,
...
liveRoomsRouter.get('/:id', optionalAuth, async (req, res) => {
    ...
    let hasRsvp = false;
    const uid = res.locals.authUserId;
    if (uid && mongoose.Types.ObjectId.isValid(uid)) {
        hasRsvp = Boolean(await LiveRoomRsvp.exists({
            roomId: room._id,
...
liveRoomsRouter.post('/:id/token', requireAuth, async (req, res) => {
    ...
    const userName = user?.fullName?.trim() || user?.username || 'Guest';
...
//# sourceMappingURL=live-rooms.js.map