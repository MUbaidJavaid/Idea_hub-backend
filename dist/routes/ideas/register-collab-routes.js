...
function collabToApi(doc, requester) {
    ...
    return {
        ...
        respondedAt: respondedAt instanceof Date ? respondedAt.toISOString() : undefined,
...
export function registerCollabRoutes(ideasRouter) {
    ideasRouter.post('/:id/collab-request', requireDb, requireAuth, async (req, res) => {
        ...
        try {
            ...
            const title = ideaTitle?.title?.slice(0, 80) ?? 'your idea';
            await Notification.create({
                ...
                body: `${requester.username} wants to collaborate on “${title}”.`.slice(0, 500),
                isRead: false,
                isPushSent: false,
    ...
    ideasRouter.get('/:id/collab-requests', requireDb, requireAuth, async (req, res) => {
        const { id: ideaId } = req.params;
        const userId = res.locals.authUserId;
        if (!userId || !mongoose.Types.ObjectId.isValid(userId)) {
            res.status(401).json({
    ...
    ideasRouter.patch('/:id/collab-requests/:reqId', requireDb, requireAuth, async (req, res) => {
        ...
        const body = req.body;
        const status = body.status === 'accepted' || body.status === 'rejected'
            ? body.status
            : null;
        const responseMessage = typeof body.responseMessage === 'string' ? body.responseMessage.trim() : '';
        if (!status) {
            res.status(400).json({
                success: false,
        ...
        if (status === 'accepted') {
            const requesterOid = request.requesterId;
            const has = (idea.collaborators ?? []).some((c) => String(c.userId) === String(requesterOid));
            if (!has) {
        ...
        if (status === 'accepted') {
            await Notification.create({
                ...
                body: `Your collaboration request for “${ideaTitleShort}” was accepted.`.slice(0, 500),
                isRead: false,
                isPushSent: false,
                metadata: {},
            });
            ...
            await Notification.create({
                ...
                body: `Your collaboration request for “${ideaTitleShort}” was declined.`.slice(0, 500),
                isRead: false,
                isPushSent: false,
                metadata: {},
            });
...
//# sourceMappingURL=register-collab-routes.js.map