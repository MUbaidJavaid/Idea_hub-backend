...
import { createInitialIdeaVersion, } from '../../lib/idea-versioning.js';
import { Idea, User } from '../../models/index.js';
import { FREE_TIER_IDEAS_PER_MONTH, getEffectivePlan, scanJobPriorityForUser, startOfUtcMonth, } from '../../lib/subscription.js';
import { CATEGORIES, MEDIA_TYPES } from './constants.js';
import { mapIdeasForPublicApi } from './map-public.js';
export async function postCreateIdea(req, res) {
    ...
    const parentIdeaIdStr = typeof parentRaw === 'string' && mongoose.Types.ObjectId.isValid(parentRaw.trim())
        ? parentRaw.trim()
        : '';
    ...
    for (const item of mediaIn) {
        ...
        const m = item;
        const cdnUrlRaw = typeof m.cdnUrl === 'string' ? m.cdnUrl.trim() : '';
        const firebaseUrl = typeof m.firebaseUrl === 'string' ? m.firebaseUrl.trim() : '';
        const deliveryUrl = cdnUrlRaw || firebaseUrl;
        const publicId = typeof m.publicId === 'string' ? m.publicId.trim() : '';
        const mimeType = typeof m.mimeType === 'string' ? m.mimeType.trim() : '';
        const mediaType = typeof m.mediaType === 'string' ? m.mediaType.trim() : '';
        if (!deliveryUrl || !mimeType || !MEDIA_TYPES.has(mediaType)) {
            res.status(400).json({
                success: false,
                message: 'Each media item needs cdnUrl (or legacy firebaseUrl), mimeType, and mediaType',
                data: null,
            });
            return;
        }
        const thumbnailUrl = typeof m.thumbnailUrl === 'string' ? m.thumbnailUrl.trim() : '';
        const fileSizeBytes = typeof m.fileSizeBytes === 'number' && m.fileSizeBytes >= 0
            ? m.fileSizeBytes
            : 0;
        mediaDocs.push({
            mediaType,
    ...
    try {
        ...
        if (String(process.env.ENABLE_GAMIFICATION ?? '').toLowerCase() === 'true' &&
            fresh.status === 'published') {
            const { onIdeaPublished, onIdeaDuetPublished } = await import('../../services/gamification.service.js');
        ...
        if (String(process.env.ENABLE_AI_COACH ?? '').toLowerCase() === 'true' &&
            fresh.status === 'published') {
...
//# sourceMappingURL=post-create-idea.js.map